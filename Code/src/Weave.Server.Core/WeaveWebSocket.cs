   ...
   public class WeaveWebServer : WeaveBaseServer
    {
        ...
        protected override byte[] packageData(byte[] alldata, Socket soc, SslStream Stream, byte[] tempDataList,DateTime lasttime)
        {
            if (alldata.Length > 0)
            {
           ...
           return base.packageData(alldata, soc, Stream, tempDataList, lasttime);
         }
