...
namespace Weave.Base.WeaveBase
{
    ...
    {
        public bool Asynexecution = true;
        ...
        public byte[] packageDatabtye(byte[] alldata, Socket soc, WeaveReceiveBitEvent ReceiveBitEventHandercback,
            SslStream ssl, WeaveReceiveSslbitEvent ReceiveBitEventHandercbackssl,DateTime lastdt)
        {
            try
            {
                //int i = 0;
                    ...
                    if (bytesRead > 2)
                    {
                        ...
                        else if (bytesRead > 4 + a)
                        {
                            ...
                            else
                            {
                                ...
                                alldata = temps;
                                if ((DateTime.Now - lastdt).TotalSeconds > 2)
                                    return new byte[0];
                            ...
                            {
                                if ((  DateTime.Now- lastdt).TotalSeconds > 60)
                                    return new byte[0];
                                return alldata;
                            }
                            ...
                            try
                            {
                                ...
                                if (ssl == null)
                                {
                                    ...
                                    if (Asynexecution)
                                    {
                                        //System.Threading.ThreadPool.UnsafeQueueUserWorkItem(new WaitCallback( ReceiveBitEventHandercback), me);
                                        Task.Run(() =>
                                        {
                                            ReceiveBitEventHandercback.Invoke(tempbtye[0], bs, soc);
                                        });
                                    }
                                    else
                                ...
                                else
                                {
                                    if (Asynexecution)
                                        Task.Run(() =>
                                    {
                                        ReceiveBitEventHandercbackssl.Invoke(tempbtye[0], bs, ssl);
                                    });
                                    else
                                        ReceiveBitEventHandercbackssl(tempbtye[0], bs, ssl);
                                }
                        ...
                        else if (bytesRead > 2 + a)
                        {
                            ...
                            try
                            {
                                ...
                                else
                                {
                                    ...
                                    else ReceiveBitEventHandercbackssl?.Invoke(tempbtye[0], temp2, ssl);
                                }
