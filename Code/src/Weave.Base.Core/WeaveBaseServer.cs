...
namespace Weave.Base
{
   ...
   //     public static ManualResetEvent allDone = new ManualResetEvent(false);
        ...
        public event WeaveReceiveSslbitEvent WeaveReceiveSslBitEvent;

        public int Port { get; set; }
        public byte defaultCommand = 0x0;
        public X509Certificate2 Certificate { get; set; }
        public SslProtocols EnabledSslProtocols { get; set; }
        public bool Asynexecution { get => asynexecution; set => myencoder.Asynexecution=asynexecution = value ;  }

        protected string loaclip;
        /// <summary>
        /// 同步/或/异步回调数据事件
        /// </summary>
        private bool asynexecution=true;
        ...
        public virtual void Stop()
        {
            ...
            try
            {
                ThreadAcceptHander.Abort();
                ThreadReceiveHander.Abort();
                // ThreadReceivePageHander.Start();
                ThreadKeepAliveHander.Abort();
            }
            catch { }
        }
     //   WaitCallback ReceiveBitEventHandercback, ReceiveEventHandercback;
        Thread ThreadAcceptHander;
        Thread ThreadKeepAliveHander;
        public virtual void Start(int port)
        {
            ...
            isrun=true;
             ThreadAcceptHander = new Thread(new ParameterizedThreadStart(AcceptHander));
            ...
            //Thread ThreadReceivePageHander = new Thread(new ParameterizedThreadStart(ReceivePageHander));
             ThreadKeepAliveHander = new Thread(new ParameterizedThreadStart(this.KeepAliveHander));
      ...
      protected virtual   void KeepAliveHander(object obj)
        ...
        protected virtual byte[] packageData(byte[] alldata, Socket soc, SslStream ssl,byte[] tempDataList=null,DateTime lasttime=new DateTime())
        {
            ...
            try
            {
                ...
                {
                    aall= myencoder.packageDatabtye(alldata, soc, weaveReceiveBitEvent, ssl, WeaveReceiveSslBitEvent, lasttime);
                }
                if (weaveDataType == WeaveDataTypeEnum.custom)
                {
                    if (alldata.Length > 0)
                    {
                        ...
                        if (ssl == null)
                        {
                            if (Asynexecution)
                            {
                                ...
                                //System.Threading.ThreadPool.UnsafeQueueUserWorkItem(    ReceiveBitEventHander, me);
                                Task.Run(() =>
                               {
                        ...
                        else
                        {
                            if (Asynexecution)
                            {
                                Task.Run(() =>
                            {
                                if (WeaveReceiveSslBitEvent != null)
                                    WeaveReceiveSslBitEvent?.Invoke(defaultCommand, alldata, ssl);
                            });
                            }
                            else
                            {
                ...
                try
                {
                    ...
                    if (bytesRead <= 0)
                    {
                        if(workItem.allDataList.Length>3)
                          workItem.allDataList = packageData(workItem.allDataList, workItem.SocketSession, workItem.Stream, workItem.tempDataList, workItem.Lasttime);
       ...
       protected  bool Sendbyte(Socket socket, byte[] text)
        ...
        void packageDataHander(WeaveNetWorkItems netc)
        {
           ...
           // Console.WriteLine("packageDataHander:" + netc.allDataList.Length);
            netc.allDataList = packageData(netc.allDataList, netc.SocketSession, netc.Stream, netc.tempDataList, netc.Lasttime);
        ...
        void packageDataHanderobj(object obj)
        {
            ...
            try
            {
              lb1222:
                ...
                if (netc.SocketSession.Connected)
                {
                ...
                {
                    if ( netc.allDataList.Length >= 5 && templen != netc.allDataList.Length)
                    {
                        goto lb1222;
                    }
                }
          ...
          //  StringBuilder messageData = new StringBuilder();
            ...
            do
            {
                try
                {
                    ...
                    weaveDeleteSocketListEvent(netc.SocketSession);
                    try
                    {
                        weaveNetworkItems.Remove(netc);
                    }
                    catch (Exception EX_NAME)
                    {
                        // Console.WriteLine(EX_NAME);
        ...
        void AcceptHander(object ias)
        {
            ...
            while (isrun)
            {
                ...
                if (Certificate != null)
                {
                    ...
                    netc.Stream.AuthenticateAsServer(Certificate, false, SslProtocols.Default, false);
